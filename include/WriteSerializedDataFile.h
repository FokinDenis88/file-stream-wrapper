...
namespace file {
    ...
    // object_size - количество байт, которые занимает объект
    template<typename ByteT = unsigned char, typename SymbolT>
    void WriteSerializedDataFile(const char* file_path, const SymbolT* const first_object_symbol,
    ...
    template<typename ByteT = unsigned char, typename SymbolT>
    void WriteSerializedDataFile(const std::string& file_path, const SymbolT* const first_object_symbol,
                                 const std::streamsize object_size = sizeof(SymbolT),
                                 std::ios_base::openmode open_mode = OpenModeWriteBinaryAppend,
