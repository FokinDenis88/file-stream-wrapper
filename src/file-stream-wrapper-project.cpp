...
int main() {
    ...
    catch (file::ErrorOpenFile& error) {
        std::cerr << '\n' << error.what();
        return -1;
    }
    catch (const file::ErrorCloseFile& error) {
        std::cerr << '\n' << error.what();
        return -1;
    }
    catch (const file::ErrorWriteFile& error) {
        std::cerr << '\n' << error.what();
        return -1;
    }
    catch (const file::ErrorReadFile& error) {
        std::cerr << '\n' << error.what();
        return -1;
    }
    catch (const file::ErrorFileOperation& error) {
        std::cerr << '\n' << error.what();
        return -1;
    }
